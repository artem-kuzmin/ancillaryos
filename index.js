...
const carts = {};
const conversations = {}; // для пошагового ввода бронирования
...
bot.command('addproperty', async (ctx) => {
  ...
  if (parts.length < 2) {
    return ctx.reply('Формат: /addproperty Название | Адрес');
  }

  const { data, error } = await supabase.from('properties').insert({
    pm_telegram_id: ctx.from.id,
    name: parts[0],
    address: parts[1]
  }).select().single();
...
bot.command('addancillary', async (ctx) => {
  ...
  if (parts.length < 4) {
    return ctx.reply('Формат: /addancillary ID_объекта | Название услуги | цена_будни | цена_выходные');
  }

  const { data, error } = await supabase.from('ancillaries').insert({
    property_id: parts[0],
    name: parts[1],
    price_weekday: parseInt(parts[2]),
    price_weekend: parseInt(parts[3])
  }).select().single();
...
bot.command('newbooking', async (ctx) => {
    ...
    if (!properties || properties.length === 0) {
      return ctx.reply('Сначала добавьте объект: /addproperty');
    }
    ...
    const buttons = properties.map(p => [
      Markup.button.callback(p.name, `nb_${p.id.slice(0, 8)}`)
    ]);
  ...
  bot.on('text', async (ctx) => {
    ...
    if (!conv) return; // не в процессе ввода — игнорируем
    ...
    if (conv.step === 'checkin_date') {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) {
        return ctx.reply('Неверный формат. Введите дату как: 2026-05-01');
      }
    ...
    if (conv.step === 'checkout_date') {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) {
        return ctx.reply('Неверный формат. Введите дату как: 2026-05-05');
      }
  
      const { data, error } = await supabase.from('bookings').insert({
        property_id: conv.propertyId,
        guest_name: conv.guestName,
        checkin_date: conv.checkinDate,
        checkout_date: text
      }).select().single();
...
const TEMPLATES = [
  ...
  bot.command('templates', async (ctx) => {
    ...
    const buttons = properties.map(p => [
      Markup.button.callback(p.name, `tpl_${p.id.slice(0, 8)}`)
    ]);
...
bot.start(async (ctx) => {
  ...
  const { data: booking } = await supabase.from('bookings')
    .select('*, properties(name, address, checkin_time, checkout_time)')
    .eq('id', bookingId)
    .single();
  ...
  await supabase.from('bookings')
    .update({ guest_telegram_id: ctx.from.id })
    .eq('id', bookingId);
  ...
  // Store full data in memory, use short keys for buttons
  const cartKey = ctx.from.id.toString();
  carts[cartKey] = { ancillaries, bookingId };
...
bot.action(/buy_(\d+)_(\d+)/, async (ctx) => {
  ...
  ctx.answerCbQuery();
  ctx.reply(
    `Вы выбрали: *${ancillary.name}* — ₽${price.toLocaleString()}\n\nПодтвердить покупку?`,
    {
      parse_mode: 'Markdown',
...
bot.action(/ok_(\d+)_(\d+)/, async (ctx) => {
  ...
  const { error } = await supabase.from('purchases').insert({
    booking_id: cart.bookingId,
    ancillary_id: ancillary.id,
    price_paid: price,
    status: 'paid'
  });
  ...
  const { data: booking } = await supabase.from('bookings')
    .select('guest_name, properties(pm_telegram_id, name)')
    .eq('id', cart.bookingId)
    .single();

  if (booking) {
    bot.telegram.sendMessage(
      booking.properties.pm_telegram_id,
